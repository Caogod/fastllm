...
Thought: you should split the question to single task and think about what to do
...
def text_complete_last_message(history, stop_words_ids, gen_kwargs):
    ...
    _stop_words_ids = [tokenizer.encode(im_end)[0]]
    if stop_words_ids:
        for s in stop_words_ids:
            _stop_words_ids.append(s)
    stop_words_ids = _stop_words_ids
    output = model.response(prompt, **gen_kwargs, stop_token_ids=stop_words_ids)
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    stop_words_ids = [tokenizer.encode(s)[0] for s in stop_words] if stop_words else None
    # query, _tokens = make_context(tokenizer, query, history)
    ...
    new_tokens = tokenizer.encode(response)
    # query, _tokens = make_context(tokenizer, query, history)
    usage=UsageInfo(
        prompt_tokens=0,
        total_tokens=0+len(new_tokens),
...
async def predict(
    ...
    yield "{}".format(_dump_json(chunk, exclude_unset=True, ensure_ascii=False))
    ...
    response_generator = model.stream_response(
        query, history=[], **gen_kwargs,
        one_by_one=False,
        stop_token_ids=stop_words_ids
    )
    ...
    for new_response in response_generator:
        ...
        usage.total_tokens += len(new_tokens)
...
if __name__ == "__main__":
    ...
    args = _get_args()
    args.checkpoint_path='/home/modelhub/qwen/qwen-14b-int4.flm'
    args.server_port=18011
    tokenizer = AutoTokenizer.from_pretrained(
        "/home/modelhub/qwen/Qwen-14B-Chat-Int4",
